package InfluxAdConsumer.org.example;

import InfluxAdConsumer.org.example.serde.AppSerdes;
import InfluxAdConsumer.org.example.serde.JsonDeserializer;
import InfluxAdConsumer.org.example.types.WindowAgg;
...
public class InfluxdbAdConsumerFin {
    ...
    private static Consumer<Windowed<String>, WindowAgg> createConsumer() {
        ...
        consumer.subscribe(Arrays.asList(AppConfigs.adGroupbyName));
    ...
    static void runConsumer() throws InterruptedException {
        ...
        while (true) {
            ...
            for (ConsumerRecord<Windowed<String>, WindowAgg> record : records) {
                ...
                String adAccountId = groupedKey[0];
                String creativeId = groupedKey[1];
                ...
                //influxdb에 insert
                influxDB.write(Point.measurement("ad")
                        .time(eventTime, TimeUnit.MILLISECONDS)
                        .tag("adAccountId", adAccountId)
                        .tag("creativeId", creativeId)
                        .addField("win", win)
