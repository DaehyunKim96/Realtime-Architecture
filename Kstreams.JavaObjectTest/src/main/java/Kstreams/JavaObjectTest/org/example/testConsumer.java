...
import Kstreams.JavaObjectTest.org.example.types.WindowAgg;
...
//        KafkaConsumer<Windowed<String>, WindowAgg> consumer = new KafkaConsumer<>(props,windowedDeserializer , AppSerdes.WindowAgg().deserializer());
        KafkaConsumer<String, WindowAgg> consumer = new KafkaConsumer<>(props);
...
//            ConsumerRecords<Windowed<String> , WindowAgg> records = consumer.poll(Duration.ofSeconds(10));
            ConsumerRecords<String , WindowAgg> records = consumer.poll(Duration.ofSeconds(10));
//            for(ConsumerRecord<Windowed<String>, WindowAgg> record : records) {
            for(ConsumerRecord<String, WindowAgg> record : records) {
...
//                System.out.println(createdtime);
                System.out.println(record.value().getWin());
                System.out.println(record.value().getClick());
